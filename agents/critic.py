...
def critic_node(state: InsightFlowState) -> dict:
    ...
    # get the latest analysis attempt
    latest_attempt = state["analysis_history"][-1]

    # get quality report for cross-checking
    ...
    Tool result: {safe_json(latest_attempt.tool_result)}
    ...
    # force approve if max attempts reached — use best result available
    ...
    if verdict.approved:
        ...
        trace_entry = (
            f"CRITIC: approved — "
            f"confidence {verdict.confidence_score*100:.0f}% — "
            f"{verdict.reason[:60]}"
        )
    else:
        next_agent = "analyst"
